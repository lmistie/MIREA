 ...
 *
 *    Коррекция *AB*:
 *    Добавлена функция void DelayResolution100us(Int32U Dely)
 *    Добавлена функция void GLCD_DisplayClear(void)
 *    Добавлена функция void LCD_print (const char *fmt,...)
 *    Контраст по умолчанию = 0x37
 *    Для компилятора MDK-ARM: - добавлен заголовочный stdio.h
 *                             - введены имена блоков union, struct 
 *                             - объявления переменных - в начале функции
 *
 *    Коррекция от 05.2019:
 *    В функции GLCD_PowerUpInit проверяется несколько ID производителей дисплея
 *      или проверка блокируется по признаку GLCD_DisID.
 *    Добавлена функция GLCD_GetID()
 *
 **************************************************************************/
#include "drv_glcd.h"
#include "glcd_ll.h"
...
// Признак запрета идентификации дисплея по ID производителя (для отладки)
int GLCD_DisID = 0;

// Команды контроллера Philips PCF8833
...
// Команды контроллера Epson S1D15G00
  ...
  if(Write2_DRAM)
  {
	...
	if(Glcd_Iss==GLCD_ISS_0)
	{
      ...
      pDATCTR_Data = (pDATCTR_Data_t)pData;
			// MADCTR + COLMOD
			// Data reconstruction
      DataMode.un.st.MY = pDATCTR_Data->unRC.stRC.MY;
			DataMode.un.st.MX = pDATCTR_Data->unRC.stRC.MX;
			DataMode.un.st.MV = pDATCTR_Data->unRC.stRC.MV;
			DataMode.un.st.ML = pDATCTR_Data->unRC.stRC.ML;
			DataMode.un.st.RGB = !pDATCTR_Data->unRGB.stRGB.RGB;
    ...
    case DATCTR:
      pDATCTR_Data = (pDATCTR_Data_t)pData;
      pDATCTR_Data->unRC.stRC.ML  = 0;
			pDATCTR_Data->unRC.stRC.MX = !pDATCTR_Data->unRC.stRC.MX;
      // 4K colors Type A
      pDATCTR_Data->unGS.stGS.GS  = 2;
      break;
    case SETCON:
...
GLCD_Status_t GLCD_PowerUpInit (pInt8U pInitData)
{
  ...
  Glcd_Iss = GLCD_ISS_0;
  GLCD_SendCmd(RDDID,(pInt8U)&Data,0);
  if (!((Data & 0xFF) == 0xFE ||        //Если дисплей не PCF8833
        (Data & 0xFF) == 0x29 ||
        ((Data >> 16) & 0xFF) == 0x83 ||
        ((Data >> 16) & 0xFF) == 0x8C
       ) && !GLCD_DisID                 // и нет блокировки,
     )
  {                                     // выбор как S1D15G00
  	Glcd_Iss = GLCD_ISS_1;
  }
  ...
  put_constant_in_var = GLCD_DEF_DPL_INV;
...
#error "PARTIAL_MODE not supported yet"
  ...
  pDATCTR_Data->unRGB.stRGB.RGB = GLCD_DEF_DPL_RGB_ORD; // BGR / RGB order
  pDATCTR_Data->unRC.stRC.MY = GLCD_DEF_DPL_MY; 			  // no mirror Y
  pDATCTR_Data->unRC.stRC.MX = GLCD_DEF_DPL_MX; 			  // no mirror X
  pDATCTR_Data->unRC.stRC.MV = GLCD_DEF_DPL_V; 				  // vertical RAM write in Y/X direction
  pDATCTR_Data->unRC.stRC.ML = GLCD_DEF_DPL_ML; 			  // line address order (bottom to top) / (top to bottom)
...
/*************************************************************************
 * Очистка дисплея
 * back - 0 (по умолчанию) светлый фон, 1 - темный фон
 *************************************************************************/
void GLCD_DisplayClear(int back)
{
  Int32U Data, i;
  Data = 0 | ((GLCD_HORIZONTAL_SIZE - 1) << 8);
  GLCD_SendCmd(CASET, (pInt8U)&Data, 0);\
  ...
  GLCD_SendCmd(RASET, (pInt8U)&Data, 0);
  Data = back ? 0x000 : DEF_BACKGND_COLOUR;
  for (i = 0; i < (GLCD_HORIZONTAL_SIZE * GLCD_VERTICAL_SIZE); ++i)
  {
    LCD_WRITE_PIXEL(Data);
  }
